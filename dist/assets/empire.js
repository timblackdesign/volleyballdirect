...
jquery$1(window).on('dragStart', function (event) {
  ...
  if ($flickityEnabledEl.length) {
    var flickity = index.data($flickityEnabledEl[0]);
...
var Quantity = function () {
  function Quantity(el) {
    ...
    this.stockLimit = this.$el.data('stock-limit') || false;
  ...
  createClass(Quantity, [{
    ...
    value: function _validateValue(event) {
      ...
      var newValue = isNaN(baseValue) ? 1 : baseValue;

      var isValid = true;
      var value = newValue;

      // Check if there is a stock limit on the product
      if (this.stockLimit) {
        var limit = parseInt(this.stockLimit, 10);
        isValid = newValue <= limit;

        value = Math.min(limit, newValue);

        if (!isValid) {
          this.$el.trigger('quantity:error', { limit: limit });
        }
      }
    ...
    value: function _watchSelect(event) {
      var _validateValue2 = this._validateValue(event),
          value = _validateValue2.value,
          isValid = _validateValue2.isValid;

      // Update select with sanitized value
    ...
    value: function _watchInput(event) {
      var _validateValue3 = this._validateValue(event),
          value = _validateValue3.value;
...
var index$1 = function (parentSelector, opts) {
...
var RichText = function () {
  ...
  createClass(RichText, [{
    key: 'unload',
    value: function unload() {
      // Iterate backwards through the tab groups to remove event binding
      for (var i = this.tabGroups.length - 1; i >= 0; i--) {
        var events = this.tabGroups[i];
        events.forEach(function ($el) {
          return $el.off('.tabs');
        });

        this.tabGroups.splice(i, 1);
      }
    }
    ...
    /**
     * Bind each grouping of tabs
     *  - If no active tab is defined, set the first tab as active
     *
     * @param el
     * @private
     */
    ...
    key: '_initTabs',
    value: function _initTabs(el) {
      var $tabsHeader = jquery$1(el);
      var $tabsContent = $tabsHeader.next('.tabs-content');

      if (!$tabsHeader.find('li.active').length) {
        $tabsHeader.find('li:first').addClass('active');
      }

      if (!$tabsContent.find('li.active').length) {
        $tabsContent.find('li:first').addClass('active');
      }

      var events = [$tabsHeader.on('click.tabs', 'li', this._switchTab), $tabsHeader.on('click.tabs', 'a', function (event) {
        return event.preventDefault();
      })];

      this.tabGroups.push(events);
    }
    ...
    key: '_switchTab',
    value: function _switchTab(event) {
      var $target = jquery$1(event.currentTarget);
      var $tabsHeader = $target.parent('.tabs');
      var $tabsContent = $tabsHeader.next('.tabs-content');

      $target.addClass('active').siblings().removeClass('active');

      $tabsContent.find('> li').eq($target.index()).addClass('active').siblings().removeClass('active');
    }
  }]);
  return RichText;
}();
...
var CurrencyConverter = function () {
  ...
  _createClass(CurrencyConverter, [{
    ...
    value: function update(priceEl) {
      ...
      var formattedPrice = window.Currency.formatMoney(this._moneyValue(priceEl.innerHTML), this.options.moneyFormat);
...
var index$2 = CurrencyConverter$1;
...
var StickyHeader = function () {
  function StickyHeader(options, settings) {
    ...
    this.headerMainEl = this.$el[0].querySelector('[data-site-header-main]');
    this.headerNavEl = this.$el[0].querySelector('.site-navigation-wrapper');
  ...
  createClass(StickyHeader, [{
    ...
    key: '_stickyOffset',
    value: function _stickyOffset() {
      var offset = this.headerMainEl.getBoundingClientRect().height;

      if (layout.isBreakpoint('L')) {
        offset = offset + this.headerNavEl.getBoundingClientRect().height;
      }
      ...
      this.$el.data('header-offset', offset);
      this.$siteMain.css('paddingTop', offset);
    }
  }, {
    key: '_slideToggle',
    value: function _slideToggle() {
      if (this.$el.hasClass(this.navOpenClass)) {
    ...
    value: function _toggleStickyHeader() {
      var offset = parseInt(this.$el.data('header-offset'), 10) || 200;
      var scrollThreshold = window.pageYOffset > offset + 20;

      // Sticky header is scrolled, is and is visible -- nothing more to do!
      if (scrollThreshold && this.$body.hasClass(this.scrolledClass)) {
        return;
      }
...
var NavMobileSubMenus = function () {
  ...
  createClass(NavMobileSubMenus, [{
    ...
    value: function _selectSubMenu($target) {
      var $menu = $target.next(this.subMenu);
    ...
    value: function _openSubmenu($target) {
      var _selectSubMenu2 = this._selectSubMenu($target),
          $menu = _selectSubMenu2.$menu,
          $parent = _selectSubMenu2.$parent;

      $target.addClass(this.activeMenuClass);
      this.Accordion.toggle($menu);
    }
  }, {
    key: '_closeSubmenu',
    value: function _closeSubmenu($target) {
      var _selectSubMenu3 = this._selectSubMenu($target),
          $menu = _selectSubMenu3.$menu,
          $parent = _selectSubMenu3.$parent;

      $target.removeClass(this.activeMenuClass);
      this.Accordion.toggle($menu);
...
var NavDesktop = function () {
  function NavDesktop(options) {
    ...
    this.$megaNavTrigger = this.$el.find('[data-navmenu-meganav-trigger]');
    ...
    this.$el.on('mousedown.navmenu', '.navmenu-link', function (event) {
      ...
      $target.addClass(_this.activeItem);
  ...
  createClass(NavDesktop, [{
    ...
    value: function onSectionBlockSelect(block) {
      if (this.$megaNavTrigger.length) {
        this.postMessage('stickyheader:open');

        this.openItem(this.$megaNavTrigger[0], block);
      }
    }
    ...
    value: function openItem(el) {
      ...
      $menu.parent(this.navTrigger).find('.navmenu-link-parent:first').addClass(this.activeMenu);
    ...
    value: function closeItem(el) {
      ...
      // Add an active class to the menu's parent
      $menu.parent(this.navTrigger).find('.navmenu-link-parent:first').removeClass(this.activeMenu);
...
function ProductPrice(props) {
  ...
  var $compareAt = onSale ? ProductPriceCompareAt(price) : '';
...
var HeaderSearch = function () {
  function HeaderSearch($els, settings) {
    ...
    this.events = [this.$button.on('click.header-search', function (event) {
      event.preventDefault();
      _this._search(event);
    }), this.$input.on('keyup.header-search', index$3(this._search, 250)), this.$input.on('focus.header-search', this._searchFocused), this.$takeOverButton.on('click.header-search', this._toggleTakeOver)];
  }
...
var StaticSectionHeader = function () {
  function StaticSectionHeader(section) {
    ...
    this.$siteNavigation = this.$el.find('.site-navigation');
  ...
  createClass(StaticSectionHeader, [{
    ...
    value: function onSectionBlockSelect(block) {
      if (layout.isBreakpoint('L')) {
        this.navDesktop.onSectionBlockSelect(block);
      }
    }
  }, {
    ...
    value: function _converter() {
      ...
      this.$converter.on('change.converter', function (event) {
        ...
        index$2.setCurrency(event.currentTarget.value);
      });
    }
  }]);
...
function getHeaderOffset(header) {
  var headerNavigation = header.querySelector('[data-site-navigation]');
  var headerMain = header.querySelector('[data-site-header-main]');
  var headerRect = header.getBoundingClientRect();
  var isSticky = headerMain.hasAttribute('data-site-header-sticky');

  if (isSticky) {
    var offset = headerMain.getBoundingClientRect().height;

    if (jquery$1(header).hasClass('site-header-sticky--open')) {
      offset = offset + headerNavigation.getBoundingClientRect().height;
    }
    return offset;
  } else {
...
var MessageBanner = function () {
  ...
  createClass(MessageBanner, [{
    ...
    value: function _bindEvents($banner) {
      ...
      }), jquery$1(window).on('resize.message-banner', index$3(function () {
...
var StaticCart = function () {
  function StaticCart(section) {
    ...
    script($scripts.data('shopify-api-url'), function () {
      _this._bindEvents();
    ...
    this.$el.find('.money').each(function (index, el) {
      return index$2.update(el);
    });

    if (this.settings.shipping && this.$shipping.length) {
      script($scripts.data('shopify-countries'), function () {
        script($scripts.data('shopify-common'), function () {
          script($scripts.data('shopify-cart'), function () {
  ...
  createClass(StaticCart, [{
    ...
    value: function _bindEvents() {
      ...
      this.$el.on('quantity:error', '[data-quantity-wrapper]', index$3(function (event, data) {
        _this2._quantityError(data.limit);
      }, 30, false, true));

      this.$window.on('resize.cart-page', index$3(function () {
    ...
    value: function _editItemQuantity(event) {
      ...
      var $stockLimit = $cartItem.find('[data-stock-limit]');

      // Check if there is a stock limit on the product
      if ($stockLimit.length) {
        limit = parseInt($stockLimit.data('stock-limit'), 10);
        isValid = quantity <= limit;

        quantity = Math.min(limit, quantity);
      }

      if (!isValid && isFinite(limit)) {
        this._quantityError(limit);
      } else {
    ...
    value: function _updateCart($cartItem, quantity) {
      ...
      Shopify.changeItem(id, quantity, function (response) {
        ...
        var cartItem = response.items.filter(function (item) {
          return item.id === id;
        }) || null;

        _this3._didUpdate(response, $cartItem, cartItem[0]);
      });
    }
  }, {
    key: '_quantityError',
    value: function _quantityError(limit) {
      var errorMessage = this.settings.stock_limit.replace('** count **', limit);
    ...
    key: '_didUpdate',
    value: function _didUpdate(response, $cartItem) {
      var _this4 = this;

      var cartItem = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;

      // Reload page if item is completely removed from cart
      if (cartItem && cartItem.length === 0 && this.$el.find('[data-cartitem]').length === 1) {
      ...
      // If item has been removed, there will be no cart item
      if (cartItem) {
...
var VanillaModal = unwrapExports(index$4);
...
var Modal = function () {
  ...
  createClass(Modal, [{
    ...
    value: function _onOpen() {
      ...
      this.$window.on('resize.modal', index$3(function () {
  ...
  createClass(Node, [{
    ...
    value: function getValidOptions() {
      ...
      var _loop = function _loop(_i) {
        Object.keys(_this.neighbors[_i]).forEach(function (key) {
  ...
  createClass(VariantHelper, [{
    ...
    value: function _switchVariantSelect(variant, availableOptions) {
      ...
      var _loop = function _loop(o) {
        ...
        var $select = jquery$1(_this2.$options[o]);
        var $optionEls = $select.find('option');

        // Iterate over each option on dropdown
        $optionEls.each(function (i, option) {
          var $option = jquery$1(option);
          var value = option.value;

          // Not an available option
          $option.prop('disabled', availableOptions[o].indexOf(value) === -1).prop('selected', false);

          // Dropdown option matches variant option
          if (variant.options[o] === value) {
      ...
      for (var o = 0; o < this.product.options.length; o++) {
        _loop(o);
      }
    }
  }, {
    ...
    value: function _switchVariantRadio(variant, availableOptions) {
      ...
      for (var i = 0; i < this.product.options.length; i++) {
        var $options = this.$options.filter('[name="' + this.product.options[i] + '"]');
  ...
  createClass(AddToCartFlyout, [{
    ...
    value: function _bindEvents($flyOut) {
      ...
      this.events.push([$flyOut.on('click.atc-banner', '[data-atc-banner-close]', function (event) {
        ...
        index$3(_this2._positionFlyOut($flyOut), 250);
    ...
    value: function _positionFlyOut() {
      ...
      $flyOut.css({ top: getHeaderOffset(this.header) });
    }
  }, {
    key: '_disableButton',
...
var ProductDetails = function () {
  function ProductDetails(options) {
    ...
    if (this.$gallery.find('[data-product-image]').length) {
      this._initGallery();
    }
  ...
  createClass(ProductDetails, [{
    ...
    value: function _updateBadge(variant) {
      ...
      if (this.variantFields.$badgeMoneySaved.length) {
        ...
        index$2.update(this.variantFields.$badgeMoneySaved[0]);
      }
    ...
    value: function _initGallery() {
      ...
      if (this.$slider.find('.image--selected').length) {
        var initialIndex = parseInt(this.$slider.find('.image--selected').attr('data-product-image-index'), 10);

        flickityOptions = jquery$1.extend({}, { initialIndex: initialIndex }, flickityOptions);
      }

      this.gallery = new index(this.$slider[0], flickityOptions);
    ...
    value: function _bindGallery() {
      ...
      var $viewport = this.$slider.find('.flickity-viewport');
       ...
       */
      this.gallery.on('cellSelect', function () {
        _this2.$slider.trigger('heightUpdate.gallery');
        _this2._updateActiveSlide(_this2.gallery.selectedIndex);
      });

      this.$slider.on('heightUpdate.gallery', function () {
        _this2._setSlideHeight($viewport, jquery$1(_this2.gallery.selectedElement));
      });
      ...
      this.$window.on('resize.gallery', index$3(function () {
        _this2.$slider.trigger('heightUpdate.gallery');
      }));
    }
  }, {
    ...
    value: function _updateActiveSlide(index$$1) {
      var $newSlide = this.$slider.find('[data-product-image-index=' + index$$1 + ']');
      var $newThumbnail = this.$sliderPagination.find('[data-product-thumbnail=' + index$$1 + ']');
...
var ProductGridItem = function () {
  function ProductGridItem($el) {
    ...
    if (this.hasProductActions) {
      ...
      script(jquery$1('[data-scripts]').data('shopify-api-url'), function () {
...
var StaticGiftcard = function () {
  function StaticGiftcard(section) {
    ...
    script(jquery$1('[data-scripts]').data('shopify-qrcode'), function () {
...
var DynamicTwitterFeed = function () {
  function DynamicTwitterFeed(section) {
    ...
    this.$window.on('resize.twitter', index$3(function () {
  ...
  createClass(DynamicTwitterFeed, [{
    ...
    value: function _renderTweets(tweets) {
      ...
      this.$tweets.fadeOut().promise().then(function () {
        _this2.$content.append(tweetsArray);
        _this2.$tweets.remove();

        _this2.$wrapper.removeClass(_this2._extractWrapperClass()).addClass(wrapperClass).css('min-height', '');
...
var DynamicInstagramFeed = function () {
  function DynamicInstagramFeed(section) {
    ...
    this.$wrapper = this.$el.find('[data-instagram-wrapper]');
    this.$contentPlaceholder = this.$el.find('[data-instagram-photo-placeholder]');
  ...
  createClass(DynamicInstagramFeed, [{
    key: '_extractWrapperClass',
    value: function _extractWrapperClass() {
      var wrapperClass = this.$wrapper.attr('class').match(/instagram--photo-count-\d+$/);
    ...
    value: function _renderPhotos(photos) {
      ...
      var wrapperClass = 'instagram--photo-count-' + photos.length;
      ...
      photos.forEach(function (photo) {
        ...
        if (standardResolution.width > lowResolution.width) {
      ...
      this.$contentPlaceholder.fadeOut(function () {
        _this2.$wrapper.removeClass(_this2._extractWrapperClass()).addClass(wrapperClass);
